...
def chart(request):
    ...
    q = m_Product.objects.all().values('Brand').values('quantity').annotate(sum = Sum("quantity"))
    
    queryset = m_Product.objects.filter(brUn)
    print(queryset)
    ...
    context = {'products': products, 'pres': pres,'usertime': usertime, 'u':data, 'brun':brUn, 'brq':brq}
