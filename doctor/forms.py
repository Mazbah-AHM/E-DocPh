...
class AccountUpdateform(forms.ModelForm):
    ...
    def clean_email(self):
        if self.is_valid():
            ...
            try:
                account = Doctor.objects.exclude(pk = self.instance.pk).get(email=email)
    ...
    def clean_username(self):
        if self.is_valid():
            ...
            try:
                account = Doctor.objects.exclude(pk = self.instance.pk).get(username=username)
            except Doctor.DoesNotExist:
                return username
            raise forms.ValidationError("Username '%s' already in use." % username)


class appr(forms.ModelForm):
    class Meta:
        Model = Prescription
        fields = ['authorize']            